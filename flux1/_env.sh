...
# FLUX.1 needs a recent diffusers (>=0.30 — Flux support landed there) +
# transformers + accelerate + Pillow + sentencepiece (T5 tokenizer). The shared
# 'doll' env (hunyuanvideo-1.5 pulls diffusers==0.35.0 / transformers==4.57.1)
# already satisfies this. If your env has an OLDER diffusers, use a dedicated
# env: conda create -n flux1 python=3.10 -y && CONDA_ENV=flux1 ...
...
# (run_inference.py runs a single diffusers FluxPipeline on one card; FLUX.1
# is NOT auto-sharded across GPUs — use CPU offload OFFLOAD=model for VRAM
# instead of multi-GPU. GPU=N just picks which card.)
if [ -n "${GPU:-}" ]; then
