...
# Step 05 (3DGS reconstruction) also runs in wan22_rotate env if 2DGS CUDA
# extensions were built (INSTALL_2DGS=1); set SKIP_3DGS=1 to skip.
#
# REQUIRED:
...
#   SKIP_3DGS=1                           (skip step 05 3DGS reconstruction)
set -o pipefail
...
# --- step 05: 3DGS reconstruction (Pi3+COLMAP → 2DGS train → render+mesh) ---
if [ "${SKIP_3DGS:-0}" = "1" ]; then
    echo "⏭️ [skip 05] SKIP_3DGS=1"
else
    echo ""
    bash "$SCRIPT_DIR/05_3dgs_recon.sh"
fi
...
echo "  🌐 3DGS mesh:         $RESULTS_DIR/${OUTPUT_NAME:-rotate_360}/model/test/ours_*/mesh.ply"
