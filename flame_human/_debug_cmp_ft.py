"""_debug_cmp_ft.py — body finetune 前后 composite 对比拼图（最差帧段）。"""
import json
import os
import sys
...
def main():
    ...
    body_new = load_gaussian_ply(
        results_dir / "08b_finetune_body" / f"body_ft_p{pid}.ply")
    ...
    out_dir = results_dir / "composite_ft"
    ...
    for i in (4, 15, 29):
        ...
        h = head.tensors()
        row = [gt]
        for tag, body in (("old", body_old), ("ft", body_new)):
