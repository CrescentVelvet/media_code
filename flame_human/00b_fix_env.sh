...
# ── 1. numpy 降级 ──────────────────────────────────────────────────────────
echo "📦 1/4 numpy==1.23.5（torch 2.1.2 / chumpy 兼容线）..."
...
if python -W ignore -c "import charset_normalizer" 2>/dev/null; then
    echo "⏭️  2/4 charset_normalizer 正常，跳过"
else
...
if python -W ignore -c "import pytorch3d" 2>/dev/null; then
    echo "⏭️  4/4 pytorch3d 已装，跳过"
else
    echo "📦 4/4 pytorch3d 编译链（gcc11 + CUDA dev 头文件 + setuptools<81）..."
