...
# HunyuanVideo-1.5 needs torch>=2.6 + CUDA 12.x. The shared 'doll' env works
# once 00_setup_env.sh (INSTALL_DEPS=1) has upgraded torch in it; for an
# isolated setup use a dedicated env and set CONDA_ENV accordingly.
...
# (generate.py / train.py use cuda:0 by default.) Default: first visible.
if [ -n "${GPU:-}" ]; then
