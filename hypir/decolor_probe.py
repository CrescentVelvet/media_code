...
One-shot diagnostic: loads the model once, loops every face image in INPUT_DIR,
and per image prints/visualizes:
...
def main():
    if not INPUT_DIR:
        sys.exit("ERROR: set INPUT_DIR (folder of face images; loops all)")
    ...
    print(f"[*] {len(imgs)} images to probe -> {out}")
    ...
    print(f"\n🎉 Done: {len(imgs)} compare images -> {out}")
