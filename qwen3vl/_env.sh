...
# Qwen3-VL needs a recent transformers (>=4.55) + qwen-vl-utils + accelerate.
# These are fairly standard and usually compatible with the shared 'doll' env
# (hunyuanvideo-1.5 already pulls transformers==4.57.1). If Qwen3-VL demands a
# NEWER transformers that conflicts, use a dedicated env:
#   conda create -n qwen3vl python=3.10 -y && CONDA_ENV=qwen3vl ...
...
# (run_inference.py loads with device_map="auto", which respects the visible
# device list — set GPU=0,1 to shard a 30B model across 2 cards.)
# Default: leave unset -> all visible GPUs (device_map="auto" spreads the model).
if [ -n "${GPU:-}" ]; then
