...
def main():
    ...
    print(f"\n{'=' * 68}")
    print("👉 判读：")
    print("  注（2026-09-02 修正）：钩子打印的 z 是 pipeline 已反归一化的")
    print("  RAW latent（09a 已证明 vae.decode 期望 raw 输入：raw 版 roundtrip")
    print("  完美、norm 版低对比度）。所以基准不是 std≈1，而是 raw latent 的")
    print("  整体 std ≈ RMS(vae.config.latents_std)，对 MiniMax-H3 大约 1.93。")
    print("  配合 09d 的逐通道分析判定更准：")
    print("  · z 整体 std≈1.9、每通道 std≈1（归一化后）、无 NaN、无异常通道")
    print("      → transformer 输出在分布内，锅在 offload 解码路径 / 后处理 / 编码器")
    print("      → 进 09c（常驻 VAE 解码已存盘 latent）做二分")
    print("  · z 整体 std 显著偏离 1.9、且/或 11+ 通道 mean 漂移 >1σ")
    print("      → transformer 侧坏了：int8 量化或 packed-sequence 路由问题")
    print("      → 进 09d 看偏差形态（均匀放大 vs 少数通道爆），再决定 bf16 对照")
    print("  · 本脚本官方原样就出片干净")
