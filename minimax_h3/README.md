...
> - **g++ 太旧（C++20 JIT fallback）**：sglang JIT 融合 QKNorm+RoPE kernel 要 C++20 `<concepts>`，系统 g++ 不支持会 fallback 到非融合 Python 实现（功能正确但略慢）。00 在 INSTALL_DEPS 开头自动检测并 `conda install gxx_linux-64=12 python=3.10`（可选，失败不中断）。升级后日志里 JIT kernel 编译成功；不升级也能跑。
