...
**张数定稿**：外插视角数可配，**默认 20 个外插视角 → 40 张**。文档里出现的 160 是
外插视角数取别的值时的结果，不是固定值。
...
### ⚠️ 换 FLAME 会让判定翻转（必须改）

`knn_points(body_points, avatar_points, K=1)` 量的是 **body 点 → 最近 face 顶点**的距离，
**直接受 face mesh 顶点密度支配**，而 `max_dist` 按 bbox 算，未考虑密度：

```
表面点密度 ∝ N      平均最近邻间距 ∝ 1/√N
自研 20971 → FLAME 5023：  间距 × √(20971/5023) = × 2.04，平方距离 × 4.17
```

删除条件 `d² ≤ max_dist`。同一个 body 点：自研下 `d² = 0.3·max_dist` → 删；
FLAME 下 `d² = 4.17 × 0.3 = 1.25·max_dist` → **保留**。
**等价于删除半径缩到原来的 49%**（人头 bbox 对角线 25cm 时：7.9cm → 3.9cm）。

所以换 FLAME 后不是 gap 变大，而是**问题性质翻转**：3.9~7.9cm 一圈的 body 点被留下，
与 face mesh 重叠 → 从「脖子到肩膀空洞」变成「head/body 重叠 → 重影」
（与 09-08 查的 ghost face 同一类机制，换 FLAME 会加重）。

### 定稿

1. **切分距离改用 point-to-triangle**（点到 mesh **表面**，不是到最近顶点）
   —— 与顶点密度无关，换模型 / 换 LOD / 换回自研行为一致。PyTorch3D `point_face_distance`；
   5023 顶点 ~1 万面对几万 body 点成本可接受。语义上也更对：gap 本该定义成「离头部**表面**多远」。
2. **显式补缝**：face mesh 脖子下边界沿顶点法线**外推 2~3 cm** 生成一圈桥接高斯。
   不再指望 alpha blending / densification 兜底（已确认不可靠）。
...
### ⚠️ epoch 48 之后人脸区失去真实锚定

入口 2 把 box 内 GT 换成增强渲染图，入口 3 的 novel GT 也是增强渲染图 →
后 12 个 epoch 人脸区**全是伪 GT**，epoch 48 时已有的几何/外观偏差会被自蒸馏固化，且无回滚。

### 定稿

**epoch 48 触发前存 checkpoint + 触发后跑一次指标，变差可退回。**

## 11. 未决项（实现期待定）

| # | 项 | 说明 |
|---|---|---|
| 1 | **新仓 vs 继续 vggt_human** | 阻塞下一步目录搭建，需用户定 |
| 2 | exp 时间平滑 | 视频帧间连续，当前 loss 只有 L1 幅度正则，无帧间平滑项 |
| 3 | `λ_id` / `λ_exp` 取值 | 4.3 的 L1 正则权重，建议用实验定 |
| 4 | 4.3 收敛性 | 变量 = 7 + 300 + F×(6+100)，Adam 300 iter 是否足够 |
| 5 | 增强区 mask 的 3D 一致性 | 现为逐帧 2D bbox crop + feather；同一 3D 点在不同帧可能落在增强区内/外 |
| 6 | 人脸区伪 GT 是否掺真实锚定 | 已定「checkpoint + 回滚」兜底；是否再做 `w·伪GT + (1−w)·入口1 GT` 混合待实测 |
