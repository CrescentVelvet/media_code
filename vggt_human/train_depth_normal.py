...
# --- vggt_human: depth-normal consistency (P1-1, 从 archive/ 加回) -----------
# 接入点以官方语义为准: 深度取 render_pkg["depth"]（官方 render 只有 "depth"，
# 没有 "render_depth"/"render_normal"，archive 版 train_pose.py 正是取错 key
# 导致该约束被 except 静默吞掉、从未生效）。
# 法线由 depth 的 Sobel 梯度算出，与初始点云法线(KNN+PCA)对齐。
_DN_DIR = os.path.dirname(os.path.abspath(__file__))
if _DN_DIR not in sys.path:
...
if USE_DEPTH_NORMAL:
    ...
    print(f"🧭 depth-normal consistency ON (w={DN_WEIGHT}, "
          f"interval={DN_INTERVAL}, sample={DN_SAMPLE_POINTS})")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                ...
                if USE_DEPTH_NORMAL:
                    postfix["DN Loss"] = f"{ema_dn_for_log:.{7}f}"
