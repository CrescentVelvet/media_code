#!/usr/bin/env python3
"""Extract frames from a video to an image folder (loose PNGs).

Output folder is compatible with 01_face_enhance.sh's INPUT_DIR (loose images
auto-detected by face_enhance.py). So the full pipeline works unchanged:
...
def main():
    ...
    while True:
        ...
        if not ok:
            break
        if idx % step == 0:
            cv2.imwrite(os.path.join(OUTPUT_DIR, f"{saved:06}.png"), frame)
            saved += 1
        idx += 1
    cap.release()
