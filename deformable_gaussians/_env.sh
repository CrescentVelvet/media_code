...
if [ -n "$CA_FILE" ]; then
    ...
    : "${PIP_CERT:=$CA_FILE}"
    : "${CURL_CA_BUNDLE:=$CA_FILE}"     # curl honors this env var
...
# (python 3.7) in its README, and its two CUDA submodules
# (depth-diff-gaussian-rasterization @ filter-norm, simple-knn) must be built
# against that torch + a matching CUDA 11.6 toolkit. These VERSION pins
# CONFLICT with other algos in this repo (hypir wants torch==2.6.0,
# hunyuanvideo wants diffusers==0.35, f3g_avatar wants torch==2.1.0+cu121,
...
CONDA_ENV="${CONDA_ENV:-deformable_gaussians}"
...
# (train.py / render.py hardcode device cuda.) Default: first visible.
if [ -n "${GPU:-}" ]; then
