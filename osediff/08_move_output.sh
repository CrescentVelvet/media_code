#!/usr/bin/env bash
# 08_move_output.sh — 将推理/训练结果从 WSL Linux fs 剪切到 Windows D: 盘（释放 vhdx 空间）。
#
# WSL 专用：Linux fs (~/output/) 空间有限（vhdx），跑完 pipeline 后把结果搬到
...
#   SRC=~/output/osediff_results DST=/mnt/d/output/osediff_results bash osediff/08_move_output.sh
#   DRY_RUN=1 bash osediff/08_move_output.sh   # 预览，不实际移动
set -o pipefail
...
# 源目录（默认 RESULTS_DIR，与 01-04 一致）
SRC="${SRC:-$RESULTS_DIR}"
# 目标目录（默认 D:\output\osediff_results）
DST="${DST:-/mnt/d/output/osediff_results}"
# 训练产物也一起搬（如果存在）
...
if [ "$MOVE_EXTRA_DIRS" = "1" ]; then
    for _extra in \
        "$REPO_DIR/../osediff_results" \
        "$REPO_DIR/../osediff_experiments"; do
